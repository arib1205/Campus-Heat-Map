...
#define BROKER "192.168.0.13"   // MQTT broker URL
#define PORT 1883                           // MQTT broker port number
#define CLIENTID "1234"                     // use K64F MAC address without colons
...
#define TOPIC "ufheatmap"                        // MQTT topic
...
void messageArrived(MQTT::MessageData& md) {
  ...
  //  if (message.payloadlen == 3) {
    ...
    char str[10]="glow";
    strcat(str,(char*)CLIENTID);
    //pc.printf("%s",str);
    
    //pc.printf("%d",strcmp((char*)message.payload,"glow1234"/*str*/));
    
        /*if (strcmp((char*)message.payload,"glow") == 0)
            controlLED(red);*/
        
        if(strncmp((char*)message.payload, str, 8) == 0)
            controlLED(red);
    ...
    while(true) {
        ...
        if (switchEvent.value.v == 22 || switchEvent.value.v == 33) {
            switch(switchEvent.value.v) {
                case 22 :
                    buf[0] = 0;
                    sprintf(buf, (char*)CLIENTID);
                    //pc.printf("%s",message.payload);
                    break;
                case 33 :
                    buf[0] = 0;
                    sprintf(buf, (char*)CLIENTID);
